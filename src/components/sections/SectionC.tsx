...
const SectionC: React.FC<SectionProps> = ({ progress, onNavigateToSection }) => {
  ...
  const card2Position = sixthSmoothProgress > 0 
    ? '-30%' 
    : (fifthSmoothProgress > 0 ? '45%' : (fourthSmoothProgress > 0 ? '150%' : '200%')) // Change: 118% -> 100% (more visible)
  
  const card3Position = sixthSmoothProgress > 0 
    ? '45%' 
    : (fifthSmoothProgress > 0 ? '150%' : '200%') // Change: 118% -> 100% (more visible)
  ...
  return (
    <div style={sectionStyle}>
      ...
      <div className="absolute left-[830px] mt-[-13%] md:mt-[-10%] lg:mt-0 md:left-[850px] lg:left[700px] 2xl:left-1/2 -translate-x-1/2 book-button" style={{ 
        marginTop: viewportDimensions.width >= 1024 ? (viewportDimensions.width <= 1920 ? '-180px' : '240px') : undefined,
        opacity: 1,
